...
class IrUiMenu(models.Model):
    ...
    def _load_menus_blacklist(self):
        ...
        if company_id.via_company_type != 'nj':
        # # if self.env.user and self.env.user.company_ids[0].via_company_type != 'pa':
            res.append(self.env.ref('via_survey_monitoring.menu_monitoring_template').id)
            res.append(self.env.ref('via_survey_monitoring.menu_via_monitoring').id)
        return res
